	...
	for(;;) {
		...
		case PPM_CTRL_TYPE_WATT_NOREV_BRAKE:
			...
			if (servo_val >= 0.0) {
				...
				if (config.min_watt_enabled) {
					current = fabsf(servo_val * (config.min_watt / mc_interface_get_motor_voltage()));
					
					float current_by_min_motor_current = fabsf(servo_val * mcconf->l_current_min * config.max_watt_ramp_factor);
					
					if (current_by_min_motor_current < current){
						current = current_by_min_motor_current;
					}
					
					if (current > fabsf(mcconf->l_current_min)) {
						current = fabsf(mcconf->l_current_min);
					}
				} else {
